...
export function TrajectoryMap({ trajectory }: Props) {
  ...
  useEffect(() => {
    ...
    // Ensure the container has no stale Leaflet state from HMR
    ...
    import("leaflet").then((L) => {
      ...
      let map: any;
      try {
        map = L.map(mapRef.current!).setView([centerLat, centerLng], 14);
      } catch {
        // If Leaflet still complains, clear the id and retry once
        if (mapRef.current) {
          delete (mapRef.current as any)._leaflet_id;
          try {
            map = L.map(mapRef.current!).setView([centerLat, centerLng], 14);
      ...
      L.marker([camera_a.lat, camera_a.lng], { icon: camAIcon })
        .addTo(map)
        .bindPopup(`<div style="color:#1e293b;font-weight:600">${camera_a.name}</div><div style="font-size:11px">Camera A</div>`);

      L.marker([camera_b.lat, camera_b.lng], { icon: camBIcon })
        .addTo(map)
        .bindPopup(`<div style="color:#1e293b;font-weight:600">${camera_b.name}</div><div style="font-size:11px">Camera B</div>`);
      ...
      if (reidentified.length > 0) {
        L.polyline(
          ...
          { color: "#6366f1", weight: 3, opacity: 0.9, dashArray: "8 4" }
        ).addTo(map).bindPopup(
          `<div style="color:#1e293b;font-weight:600">${reidentified.length} vehicle(s) re-identified</div>`
        );
      }

      // Camera labels
      L.tooltip({ permanent: true, direction: "top", className: "cam-label", offset: [0, -12] })
        ...
        div.innerHTML = `
          <div style="background:rgba(15,23,42,0.9);border:1px solid #334155;border-radius:8px;padding:10px 14px;min-width:160px">
            ...
            <div style="display:flex;align-items:center;gap:6px;margin-bottom:4px">
              ...
              <span style="font-size:11px;color:#e2e8f0">${camera_a.name.split("–")[1]?.trim() ?? "View 1"}</span>
            </div>
            <div style="display:flex;align-items:center;gap:6px;margin-bottom:8px">
              <div style="width:10px;height:10px;border-radius:50%;background:#10b981"></div>
              <span style="font-size:11px;color:#e2e8f0">${camera_b.name.split("–")[1]?.trim() ?? "View 2"}</span>
            </div>
